package net.weeek.denisbaranov.xpathes;

public class LoginPageXPath {
    public static final String INPUT_EMAIL_XPATH = "//input[@placeholder='Электронная почта']";
    public static final String INPUT_PASSWORD_XPATH = "//input[@placeholder='Пароль']";
    public static final String BUTTON_ENTER_XPATH = "//button[@class='button-component']";
    public static final String ERROR_EMPTY_TEXT_WHEN_ENTER_EMAIL_OR_ENTER_PASSWORD_XPATH = "//div[@class='error']";
    public static final String ERROR_ENTER_EMAIL_OR_ERROR_ENTER_EMAIL_AND_PASSWORD_TEXT_XPATH = "//div[@class='error errors-log__error visible']";
}
